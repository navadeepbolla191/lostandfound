module.exports = async function handler(req, res) {
  ...
  if (!apiKey || !fromEmail) {
    return res.status(500).json({
      error:
        "Mail service is not configured on this deployment yet. Add RESEND_API_KEY and OTP_FROM_EMAIL in Vercel.",
    });
  }
  ...
  try {
    ...
    if (!resendResponse.ok) {
      return res.status(502).json({
        error: payload?.message || payload?.error || "Mail provider rejected the OTP email.",
      });
    }
